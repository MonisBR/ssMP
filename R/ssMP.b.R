  ...
  private = list(
    ...
    .run = function() {
      ...
      if (tipo == "media") {
        ...
        S <- self$options$S
        precision_OM <- self$options$precision_OM
        level_OM <- self$options$level_OM
        
        alpha <- 1 - level_OM
        z <- qnorm(1 - alpha / 2)
        n <- ceiling((z * S / precision_OM)^2)
        
        table <- self$results$tableOM
        
        table$setRow(rowNo = 1, values = list(parameter = "Sample size", value = as.character(n)))
        table$setRow(rowNo = 2, values = list(parameter = "Precision", value = as.character(precision_OM)))
        ...
        P <- self$options$P            
        precision_OP <- self$options$precision_OP
        level_OP <- self$options$level_OP
        
        alpha <- 1 - level_OP
        z <- qnorm(1 - alpha / 2)
        n <- ceiling((z^2 * P * (1 - P)) / (precision_OP^2))
        
        table <- self$results$tableOP
        
        table$setRow(rowNo = 1, values = list(parameter = "Sample size", value = as.character(n)))
        table$setRow(rowNo = 2, values = list(parameter = "Precision", value = as.character(precision_OP)))
        ...
        S1 <- self$options$S1
        S2 <- self$options$S2
        delta <- self$options$diff_IM
        power <- self$options$power_IM
        ...
        SC <- self$options$SC
        delta <- self$options$diff_IMSC
        power <- self$options$power_IMSC
        level_IMSC <- self$options$level_IMSC
        ...
        table$setRow(rowNo = 7, values = list(parameter = "Confidence level (%)", value = paste0(round(level_IMSC * 100, 1), "%")))
        ...
        P1 <- self$options$P1
        P2 <- self$options$P2
        delta <- self$options$diff_IP
        power <- self$options$power_IP
